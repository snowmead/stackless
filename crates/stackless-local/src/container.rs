...
pub fn connect() -> Result<Docker, ContainerError> {
    if std::env::var_os("DOCKER_HOST").is_some() {
        return Docker::connect_with_defaults().map_err(|err| ContainerError::Engine {
...
pub async fn provision_postgres(
    ...
    let name = container_name(instance, datastore);
        ...
        .await
    {
        Ok(existing) => {
            ...
            if let Some(id) = existing.id {
                destroy(instance, datastore, &id).await?;
            }
        }
    ...
    wait_pg_ready(&docker, &created.id, datastore).await?;

    let port = mapped_port(&docker, &created.id)
        .await
...
pub async fn destroy(
    ...
    match docker
        .remove_volume(
            &container_name(instance, datastore),
