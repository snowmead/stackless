...
pub const RESOURCE_KIND: &str = "integration-railway-mongo";

#[derive(Debug, Serialize)]
pub struct RailwayMongoConfig {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub region: Option<String>,
}

impl CatalogService for RailwayMongoConfig {
    const REFERENCE: &'static str = "railway/mongo";
}

#[derive(Debug)]
pub struct RailwayMongo;
...
impl FamilyResource for RailwayMongo {
    ...
    fn build_config(ctx: &ProvisionContext<'_>) -> Result<RailwayMongoConfig, IntegrationError> {
        let config = super::integration_config(ctx)?;
        Ok(RailwayMongoConfig {
...
mod tests {
    ...
    fn config_matches_catalog() {
        ...
        let failures = stackless_stripe_projects::verify_service(
            &catalog,
            &RailwayMongoConfig { region: None },
        );
        assert!(
            failures.is_empty(),
            "railway/mongo catalog gaps:\n{}",
            failures.join("\n")
        );
    }
...
"#,
    ...
    async fn provision_records_outputs() {
        ...
        let resource = RailwayMongo
            ...
            .await
            .unwrap();
        assert_eq!(resource.resource_kind, "integration-railway-mongo");
