...
pub const RESOURCE_KIND: &str = "integration-posthog";

#[derive(Debug, Serialize)]
pub struct PostHogAnalyticsConfig {}

impl CatalogService for PostHogAnalyticsConfig {
    const REFERENCE: &'static str = "posthog/analytics";
}
...
impl FamilyResource for PostHogAnalytics {
    ...
    ) -> Result<PostHogAnalyticsConfig, IntegrationError> {
        let _ = super::integration_config(ctx)?;
        Ok(PostHogAnalyticsConfig {})
    }
}
...
mod tests {
    ...
    fn config_matches_catalog() {
        ...
        let failures =
            stackless_stripe_projects::verify_service(&catalog, &PostHogAnalyticsConfig {});
        assert!(
            failures.is_empty(),
            "posthog/analytics catalog gaps:\n{}",
            failures.join("\n")
        );
    }

    const CATALOG_ENVELOPE: &str = r##"{"ok":true,"command":"projects catalog","data":{"last_updated":"2026-07-11T00:00:00Z","services":[{"id":"prvsvc_analytics","object":"v2.provisioning.provider_service_detail","provider":"prvdr_posthog","provider_name":"PostHog","service_id":"analytics","categories":["database"],"kind":"deployable","scope":"project","availability":"available","development":false,"livemode":true,"pricing":{"type":"component"},"configuration_schema":{"type":"object","required":[],"additionalProperties":false,"properties":{}}}]}}"##;
...
"#,
    ...
    async fn provision_records_outputs() {
        ...
        let resource = PostHogAnalytics
            ...
            .await
            .unwrap();
        assert_eq!(resource.resource_kind, "integration-posthog");
