...
pub const RESOURCE_KIND: &str = "integration-chroma";

#[derive(Debug, Serialize)]
pub struct ChromaDatabaseConfig {
    pub name: String,
}

impl CatalogService for ChromaDatabaseConfig {
    const REFERENCE: &'static str = "chroma/database";
}

#[derive(Debug)]
pub struct ChromaDatabase;

impl Hostable for ChromaDatabase {
    const PROVIDER: &'static str = "chroma";
...
impl FamilyResource for ChromaDatabase {
    ...
    fn build_config(ctx: &ProvisionContext<'_>) -> Result<ChromaDatabaseConfig, IntegrationError> {
        let config = super::integration_config(ctx)?;
        Ok(ChromaDatabaseConfig {
...
"#,
    ...
    async fn provision_records_outputs() {
        let runner = test_support::provision_script(
            CATALOG_ENVELOPE,
            serde_json::json!({"CHROMA_API_KEY": "val_api_key"}),
            0,
        );
        ...
        let resource = ChromaDatabase
            ...
            .await
            .unwrap();
        assert_eq!(resource.resource_kind, "integration-chroma");
