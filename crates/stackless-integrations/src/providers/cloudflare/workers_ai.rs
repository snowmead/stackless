//! Cloudflare Workers AI (`cloudflare/workers-ai`) — an account-level AI binding.
//! Same output shape as `cloudflare/workers` (confirmed by live discovery).
...
pub const RESOURCE_KIND: &str = "integration-cloudflare-workers-ai";

#[derive(Debug, Serialize)]
pub struct WorkersAiConfig {}

impl CatalogService for WorkersAiConfig {
    const REFERENCE: &'static str = "cloudflare/workers-ai";
}
...
mod tests {
    ...
    #[test]
    fn workers_ai_config_matches_catalog() {
        const FIXTURE: &str = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../stackless-stripe-projects/tests/fixtures/catalog.json"
        ));
        let catalog = stackless_stripe_projects::Catalog::from_json_envelope(FIXTURE).unwrap();
        let failures = stackless_stripe_projects::verify_service(&catalog, &WorkersAiConfig {});
        assert!(
            failures.is_empty(),
            "cloudflare/workers-ai catalog gaps:\n{}",
            failures.join("\n")
        );
    }
    ...
    const CATALOG_ENVELOPE: &str = r#"{"ok":true,"command":"projects catalog","data":{
        "last_updated":"2026-06-16T00:00:00Z","services":[{
            "id":"prvsvc_wai","object":"v2.provisioning.provider_service_detail",
            "provider_id":"prvdr_cloudflare","provider_name":"Cloudflare","service_id":"workers-ai",
...
env = { CF_SUBDOMAIN = "${integrations.ai.workers_dev_subdomain}" }
health = { path = "/health" }
[services.api.local]
run = "true"
"#,
    ...
    #[tokio::test]
    async fn provision_workers_ai_records_outputs() {
        ...
        let resource = CloudflareWorkersAi
            .provision(
                &stripe.as_dyn(),
                &test_def(),
                dir.path(),
            ...
            .await
            .unwrap();
        assert_eq!(resource.resource_kind, "integration-cloudflare-workers-ai");
