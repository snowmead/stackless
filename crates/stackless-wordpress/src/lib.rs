...
impl<R: CommandRunner> WordPressSubstrate<R> {
    ...
    /// `https://{stack}-{instance}-{service}.wordpress.com` — best-effort until
    /// Stripe returns the real site URL.
    fn origin_placeholder(site_name: &str) -> String {
        format!("https://{site_name}.wordpress.com")
    }
    ...
    async fn start_service(
        ...
        .await
        .map_err(projects_fault)?;
        let site_url = outputs.get("site_url").cloned();
        let origin = site_url
            .as_deref()
    ...
    fn validate_definition(&self, def: &StackDef) -> Result<(), SubstrateFault> {
        for service in def.services.keys() {
            ...
            if !config::is_valid_site_name(&site_name) {
                return Err(fault(WordPressError::ConfigInvalid {
                    location: format!("services.{service}"),
                    detail: format!(
                        "derived WordPress site name {site_name:?} is not DNS-safe; \
...
mod tests {
    ...
    fn resource_name_and_origin_are_dns_safe() {
        ...
        assert_eq!(
            WordPressSubstrate::<TokioRunner>::resource_name(&def, "demo", "web"),
            "atto-demo-web"
        );
        let (_dir, s) = subj();
        assert_eq!(
            s.service_origin(&def, "demo", "web"),
            "https://atto-demo-web.wordpress.com"
        );
    }
