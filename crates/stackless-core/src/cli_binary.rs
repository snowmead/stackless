...
//! [`resolve_cli_from`] because edition 2024 + `unsafe_code = "forbid"`
...
pub enum ResolveError {
    #[error("cannot resolve stackless executable: {detail}")]
    Spawn { detail: String },
...
pub fn resolve_cli() -> Result<(PathBuf, ResolveSource), ResolveError> {
    let override_bin = std::env::var_os(STACKLESS_BIN_ENV).map(PathBuf::from);
    let self_exe = std::env::current_exe().map_err(|err| ResolveError::Spawn {
...
/// Pure resolver for tests and [`resolve_cli`].
pub fn resolve_cli_from(
    ...
    Err(ResolveError::BinaryNotFound {
        detail: format!(
...
mod tests {
    ...
    fn override_wins_when_executable() {
        ...
        let (resolved, source) =
            resolve_cli_from(Some(&bin), &dir.path().join("consumer"), false, &[], None).unwrap();
    ...
    fn bad_override_is_hard_error() {
        ...
        let err =
            resolve_cli_from(Some(&missing), &dir.path().join("x"), false, &[], None).unwrap_err();
        assert!(matches!(err, ResolveError::BinaryNotFound { .. }));
    ...
    fn cli_marker_uses_self() {
        ...
        let (resolved, source) = resolve_cli_from(None, &self_exe, true, &[], None).unwrap();
    ...
    fn unmarked_self_never_returned_even_if_named_stackless() {
        ...
        let (resolved, source) = resolve_cli_from(
            None,
            &self_exe,
            false,
    ...
    fn unmarked_with_nothing_errors() {
        ...
        let err = resolve_cli_from(None, &self_exe, false, &[], None).unwrap_err();
        assert!(matches!(err, ResolveError::BinaryNotFound { .. }));
    ...
    fn well_known_cargo_bin() {
        ...
        let (resolved, source) =
            resolve_cli_from(None, &self_exe, false, &[], Some(home.path())).unwrap();
        assert_eq!(resolved, bin);
